...
public class Hashmap<K, V> implements MapSet<K, V> {
		...
		public Hashmap( Comparator<K> comp, int intsize) {
			...
			for(int i = 0; i < this.Hash.length; i++) {
			       this.Hash[i] = new BSTMap<K, V>(this.cmp);
			}
		}
		...
		public V put( K key, V value ) {
		    ...
		    /*if ((((BSTMap<K,V>) this.Hash[index]).containsKey(key) == true)) {
				V temp = ((BSTMap<K,V>) this.Hash[index]).get(key);
				((BSTMap<K,V>) this.Hash[index]).put(key, value);
				this.coll++;
				return temp;
			}*/
