...
class Analysis:
    ...
    def mean(self, headers, rows=[]):
        ...
        '''
        arr = self.data.select_data(headers,rows)
        arr1 = np.true_divide(arr.sum(axis=0),np.size(arr,0))
        return arr1
    ...
    def var(self, headers, rows=[]):
        ...
        NOTE: Loops are forbidden!
        '''
        arr = self.mean(headers,rows)
        arr1 = self.data.select_data(headers,rows)
        arr2 = np.subtract(arr1,arr)
        arr2 = np.square(arr2)
        arr3 = arr2.sum(axis=0)/np.size(arr2,0)
        return arr3
    ...
    def scatter(self, ind_var, dep_var, title):
        ...
        axes.scatter(arr[:,0], arr[:,1], color = 'r', marker = 'o')
        fig.suptitle(title)
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        if(len(data_vars)>1):
            for ind in range(len(data_vars)):
                axes[ind,0].set_ylabel(data_vars[ind], fontsize = (fig_sz[0]/len(data_vars))*7)
                axes[len(data_vars)-1,ind].set_xlabel(data_vars[ind], fontsize = (fig_sz[0]/len(data_vars))*7)
