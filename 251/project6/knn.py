...
class KNN:
    ...
    def __init__(self, num_classes):
        '''KNN constructor; sets the number of classes (int: num_classes) this classifier
    ...
    def predict(self, data, k):
        ...
        y_pred = []
        for pt in data:
            dists0 = pt-self.exemplars
            dists = np.linalg.norm(dists0,axis=1)
            idx = np.argsort(dists)
            C_sorted = self.classes[idx]
    ...
    def accuracy(self, y, y_pred):
        ...
        '''
        correct = np.sum(np.equal(y,y_pred))
    ...
    def plot_predictions(self, k, n_sample_pts):
        ...
        pred_data = np.column_stack((x.flatten(),y.flatten()))
        y_pred = self.predict(pred_data,k).reshape((n_sample_pts,n_sample_pts))
        cmap=ListedColormap(colors=cartocolors.qualitative.Safe_4.mpl_colors,name="Safe_4")
